...
class QualityChecker:
    ...
    def run_step(self, name: str, cmd: List[str], cwd: Optional[Path] = None, env_update: Optional[dict] = None) -> bool:
    ...
    def check_sqlx(self) -> bool:
        ...
        success = self.run_step("SQLx Prepare Check", ["cargo", "sqlx", "prepare", "--check"], 
    ...
    def run_all(self):
        ...
        if not self.fast_mode:
            ...
            # Rust Lib
            if self.run_step("Rust Lib Tests", ["cargo", "test", "--lib"], 
                            cwd=self.repo_root / 'src-tauri', 
                            env_update={'SQLX_OFFLINE': '1'}):
                self.metrics["rust_lib"] = "Pass"

            # Rust Behavior
            if self.run_step("Rust Behavior Tests", ["cargo", "test", "--tests"], 
                            cwd=self.repo_root / 'src-tauri',
                            env_update={'SQLX_OFFLINE': '1'}):
        ...
        if self.run_step("Clippy", ["cargo", "clippy", "--all-targets", "--", "-D", "warnings"], 
                         cwd=self.repo_root / 'src-tauri'):
            self.metrics["clippy"] = "Pass"

        if self.run_step("Rust Fmt", ["cargo", "fmt", "--check"], 
                         cwd=self.repo_root / 'src-tauri'):
        ...
        tsc_res = subprocess.run(["npx", "tsc", "--noEmit"], cwd=self.repo_root, capture_output=True, text=True)
    ...
    def print_report(self):
        ...
        for key, value in self.metrics.items():
            ...
            elif "errors" in value or "warnings" in value or "Uncommitted" in value or "Stale" in value:
