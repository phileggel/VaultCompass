...
- [ ] **UL — Pending entries needing user confirmation**: This spec introduces five domain terms not yet in `docs/ubiquitous-language.md`: `Cash Asset` (system Asset of class `Cash`), `Cash Holding` (Holding whose asset is a Cash Asset), `Deposit` (TransactionType variant), `Withdrawal` (TransactionType variant), `Global Value` (the `total_global_value` metric). They will be added to the UL doc with `Status: pending` (per the project rule: agents may not mark UL terms as `confirmed` without explicit user approval). User must validate before implementation begins.

None — all blocking questions have been resolved. The remaining `[ ]` item tracks UL term confirmation, which is a downstream gate for implementation but does not block contract derivation.
