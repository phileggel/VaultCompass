...
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Only `gateway.ts` calls `commands.*`   | `scripts/arch-check.py`: grep `commands\.` outside `**/gateway.ts` → fail                                                                                                                                    |
| No cross-feature imports               | same script: `features/a/**` importing `features/b/**` → fail                                                                                                                                                |
| No cross-context imports in Rust       | same script: `context/a/**` using `crate::context::b` → fail                                                                                                                                                 |
| Frontend is dumb                       | same script: arithmetic on money/quantity, date math, `.filter(...).reduce(` on domain arrays, and business conditionals in `*.tsx`/hooks → fail; allowlisted per file with a `// arch: allow <reason>` line |
| Logic coverage                         | `cargo llvm-cov` on `context/**/domain`, `context/**/application`, `use_cases/**`: **≥ 90 % lines**, fail under                                                                                              |
| Frontend coverage                      | Vitest `coverage.thresholds` on `src/features/**`: **≥ 80 % lines**, fail under                                                                                                                              |
| Stable ids on interactive elements     | script: every `<button`/`<input`/`<a` in `src/features/**` has `id=` → fail otherwise                                                                                                                        |
...
| Typed errors on the wire               | already enforced by specta + `error-model.md`; add a grep for `Result<_, String>` in `api.rs` → fail                                                                                                         |
| i18n: no literal user-facing strings   | script: string literals inside JSX text/`aria-label` in `src/features/**` → fail                                                                                                                             |
...
| 3   | Coverage thresholds            | codecov comment, advisory        | hard thresholds in the job (§ 5), codecov stays for the per-file view                                                                                                                                |
| 4   | Architecture lints             | reviewer judgement               | `scripts/arch-check.py`, run in Quality                                                                                                                                                              |
| 5   | E2E                            | main push only; local broken     | `pull_request` trigger, required check                                                                                                                                                               |
| 6   | Real-app screenshots           | none                             | `e2e/helpers/screenshot.ts`; each critical spec captures its final state; uploaded and posted on the PR                                                                                              |
