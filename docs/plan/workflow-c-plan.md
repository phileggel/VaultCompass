...
| 8   | Reviewer checks in CI                                                                 | `.github/workflows/review.yml` (lanes from the diff, Claude action per reviewer prompt, sticky comment, 🔴 fails; needs the `CLAUDE_CODE_OAUTH_TOKEN` secret, minted with `claude setup-token`, so runs bill the owner's Claude subscription)                                         |
| 9   | Visual regression                                                                     | `.github/workflows/e2e.yml`: each PR capture compared with main's last green E2E artifact (ImageMagick AE, 0.3 % threshold); regression when no frontend file changed. The visual-proof previews are not persisted, so the real-app captures are the baseline                         |
| 10  | Merge guard + branch protection                                                       | `scripts/merge.py` (open PR, every check green, required set in `required-checks.json`, a rebase that moves commits pushes and stops), `main-protection` ruleset enabled with the same checks (owner bypass kept for `just release`), `pull-requests: write` scoped to the review job |
| 11  | Scheduled runner                                                                      | cloud routine `next-todo` (created disabled), `scripts/next-todo.sh` + `just next-todo` for the laptop, `.claude/settings.json` allow/deny lists for headless runs                                                                                                                    |
| 12  | Mutation sweep                                                                        | `.github/workflows/mutants.yml` (monthly, sticky issue in techdebt format, artifact), `src-tauri/.cargo/mutants.toml` (scope = the coverage gate's logic set), `just mutants`                                                                                                         |
