...
- [ ] Code Review (`reviewer` + `reviewer-backend` + `reviewer-frontend` + `reviewer-sql`)
...
- SEL-040: Sell rows already display via `t(`transaction.type_${row.type.toLowerCase()}`)` -- just need i18n key `transaction.type_sell`
