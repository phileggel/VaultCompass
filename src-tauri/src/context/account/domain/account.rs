...
pub enum AccountChange {
    /// A new transaction was created by the user.
    TransactionInserted(Transaction),
    /// A new transaction was generated by the application on its own — a scheduled fee
    /// deduction (FEE-040); its recorded change carries the application origin (CFR-016).
...
/// Whether a holding replay enforces the ledger invariants (oversell, insufficient cash,
/// split guards) as every local write does, or tolerates them as a merged ledger must
/// (CFR-042): an inconsistent holding is kept, with its negative figures, for the user to
/// resolve (SYN-040).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum Invariants {
    /// Reject the write that would break an invariant.
    Enforced,
    /// Keep every transaction; the holding shows the break.
    Tolerated,
}
...
impl Account {
    ...
    pub fn apply_management_fee(&mut self, tx: Transaction) -> Result<Transaction> {
        self.deduct_management_fee(tx, AccountChange::TransactionInserted)
    }

    /// Applies a management fee deduction the application generated on its own (FEE-040):
    /// the same holding mechanics as `apply_management_fee`, recorded with the application
    /// origin (CFR-016).
    pub fn apply_generated_management_fee(&mut self, tx: Transaction) -> Result<Transaction> {
        self.deduct_management_fee(tx, AccountChange::TransactionGenerated)
    }

    fn deduct_management_fee(
        &mut self,
        tx: Transaction,
        inserted: fn(Transaction) -> AccountChange,
    ) -> Result<Transaction> {
        self.transactions.push(tx.clone());
        let pair_txs: Vec<&Transaction> = self
            .transactions
        ...
        self.pending_changes.push(inserted(tx.clone()));
        self.pending_changes
            .push(AccountChange::HoldingUpserted(holding.clone()));
        self.upsert_holding_in_memory(holding);
        Ok(tx)
    ...
    // -------------------------------------------------------------------------
    // Synced transactions (CFR-017/042) — applied verbatim, invariants tolerated
    // -------------------------------------------------------------------------

    /// Takes in a transaction another device recorded (CFR-017): inserts it, or replaces
    /// the one carrying the same identity, then replays the affected holding and the cash
    /// holding without the oversell and insufficient-cash guards — a merged ledger that
    /// breaks an invariant keeps every transaction and shows the holding as inconsistent
    /// (CFR-042, SYN-040).
    pub fn apply_synced_transaction(&mut self, tx: Transaction) {
        let asset_id = tx.asset_id.clone();
        let replaced = self
            .transactions
            .iter_mut()
            .find(|existing| existing.id == tx.id)
            .map(|slot| std::mem::replace(slot, tx.clone()));
        match replaced {
            Some(previous) => {
                if previous.asset_id != asset_id {
                    self.replay_holding_tolerantly(&previous.asset_id);
                }
                self.pending_changes
                    .push(AccountChange::TransactionUpdated(tx));
            }
            None => {
                self.transactions.push(tx.clone());
                self.pending_changes
                    .push(AccountChange::TransactionInserted(tx));
            }
        }
        self.replay_holding_tolerantly(&asset_id);
        self.replay_cash_holding_tolerantly();
    }

    /// Removes a transaction another device removed (CFR-017/030), replaying the affected
    /// holding and the cash holding without guards. A no-op when this account does not
    /// hold it.
    pub fn remove_synced_transaction(&mut self, tx_id: &str) {
        let Some(position) = self.transactions.iter().position(|t| t.id == tx_id) else {
            return;
        };
        let removed = self.transactions.remove(position);
        self.pending_changes
            .push(AccountChange::TransactionDeleted(tx_id.to_string()));
        self.replay_holding_tolerantly(&removed.asset_id);
        self.replay_cash_holding_tolerantly();
    }

    /// Replays one non-cash holding from its transactions with the invariants tolerated
    /// (CFR-042): a position sold below zero stays negative instead of being rejected.
    /// Removes the holding when no transaction of the pair remains (TRX-034); the cash
    /// holding is `replay_cash_holding_tolerantly`'s alone.
    fn replay_holding_tolerantly(&mut self, asset_id: &str) {
        if crate::core::cash::is_cash_asset(asset_id) {
            return;
        }
        let pair_txs: Vec<&Transaction> = self
        ...
        // A tolerated replay raises no guard, so it cannot fail.
        let Ok((holding, pnl_map)) =
            self.replay_holding(asset_id, &pair_txs, Invariants::Tolerated)
        else {
            return;
        };
        for tx in self
            ...
            .filter(|t| t.asset_id == asset_id && t.transaction_type == TransactionType::Sell)
        {
            ...
            if tx.realized_pnl != realized_pnl {
                tx.realized_pnl = realized_pnl;
                self.pending_changes
                    .push(AccountChange::TransactionUpdated(tx.clone()));
            }
        }
    ...
    /// `replay_cash_holding` with the insufficient-cash guard tolerated (CFR-042): an
    /// overdrawn balance stays negative and is shown as inconsistent (CSH-080, SYN-040).
    fn replay_cash_holding_tolerantly(&mut self) {
        let running = self
            .replayed_cash_balance(Invariants::Tolerated)
            .unwrap_or(0);
        // A tolerated replay raises no guard, so it cannot fail.
        let _ = self.upsert_cash_holding(running, Invariants::Tolerated);
    }
    ...
    fn replay_cash_holding(&mut self) -> Result<(), AccountError> {
        let running = self.replayed_cash_balance(Invariants::Enforced)?;
        self.upsert_cash_holding(running, Invariants::Enforced)
    }

    /// The cash balance the ledger replays to, walking the cash-affecting transactions
    /// chronologically. `Invariants::Enforced` raises `InsufficientCash` the moment the
    ...
    /// Upserts the Cash Holding at `running` with average_price = 1.0,
    /// total_realized_pnl = 0, last_sold_date = None — invariants from the spec entity
    /// definition. CSH-013 — the Cash Holding persists for the account's lifetime: unlike
    /// other holdings (TRX-034) it is never deleted by transaction cleanup; it stays at
    /// quantity 0 when the account holds no cash and is removed only with the account.
    /// Enforced, the holding is validated like any local write; tolerated (a synced
    /// ledger, CFR-042), an overdrawn balance is kept as the arithmetic says.
    fn upsert_cash_holding(
        &mut self,
        running: i64,
        ...
        let existing_id = self
            .holdings
            .iter()
            .find(|h| h.asset_id == cash_asset_id)
            .map(|existing| existing.id.clone());
        let holding = match (invariants, existing_id) {
            (Invariants::Enforced, Some(id)) => Holding::with_id(
                id,
                self.id.clone(),
                cash_asset_id,
                running,
            ...
            (Invariants::Enforced, None) => {
                Holding::new(self.id.clone(), cash_asset_id, running, 1_000_000, 0, None)?
            }
            (Invariants::Tolerated, id) => Holding::restore(
    ...
    fn recalculate_holding(
        ...
        self.replay_holding(asset_id, transactions, Invariants::Enforced)
    }

    /// `recalculate_holding` with the holding invariants either enforced (every local write)
    /// or tolerated (a synced ledger, CFR-042): tolerated, a sell beyond the position drives
    /// the quantity negative, a withdrawal beyond the balance overdraws it, and a split on a
    /// closed or collapsing position leaves the quantity as the arithmetic says.
    fn replay_holding(
        &self,
        asset_id: &str,
        transactions: &[&Transaction],
        ...
        const MICRO: i128 = 1_000_000;
        let enforced = invariants == Invariants::Enforced;
        ...
        let holding = match (invariants, existing_id) {
            ...
            (Invariants::Enforced, None) => Holding::new(
                self.id.clone(),
                asset_id.to_string(),
                quantity,
                average_price,
            ...
            // CFR-042 — a negative quantity is the inconsistency itself, kept as it stands.
            (Invariants::Tolerated, id) => Holding::restore(
                id.unwrap_or_else(|| Uuid::new_v4().to_string()),
                self.id.clone(),
                asset_id.to_string(),
...
pub trait AccountRepository: Send + Sync {
    ...
    /// holds none. Covers the five account-owned kinds.
    ...
    /// Every synced child of `account_id` this device holds (CFR-030): transactions, holding
    /// notes, fee schedules, and catch-up positions, with their ranks, on `conn`.
    async fn synced_children(
        &self,
        conn: &mut SqliteConnection,
        account_id: &str,
    ) -> Result<Vec<SyncedChild>>;
    /// The rank of another account carrying `name` (case-insensitive, CFR-035), on `conn`
    /// — the lowest-ranked one, then by id, when several do; `None` when no other account
    /// does or that account has never been ranked.
    ...
    /// Writes `account` verbatim — created or replaced in full — stamped with `rank`
    /// (CFR-017/014), on `conn`.
    ...
    /// Removes an account and every child it owns (CFR-030), on `conn`. A no-op when absent.
    async fn remove_account(&self, conn: &mut SqliteConnection, id: &str) -> Result<()>;
    /// `get_with_holdings_and_transactions` on `conn`.
    async fn load_aggregate(
        &self,
        conn: &mut SqliteConnection,
        id: &str,
    ) -> Result<Option<Account>>;
    /// Persists the aggregate's pending changes an applied change produced, on `conn`,
    /// recording nothing (SYN-020) and stamping `stamp`'s rank on its transaction when
    /// given. Clears `pending_changes` afterward.
    ...
    /// The account a transaction belongs to, on `conn`; `None` when the transaction is absent.
    async fn account_id_of_transaction(
        &self,
        conn: &mut SqliteConnection,
        transaction_id: &str,
    ) -> Result<Option<String>>;
    /// Writes a holding note verbatim, stamped with `rank`, on `conn` (CFR-017).
    ...
    /// Removes a holding note, on `conn`. A no-op when absent.
    ...
    /// Writes a fee schedule verbatim, stamped with `rank`, on `conn` (CFR-017).
    ...
    /// Removes a fee schedule, on `conn`. A no-op when absent.
    ...
    /// Merges a catch-up position by maximum (CFR-044), stamped with `rank`, on `conn`.
    ...
    /// Removes a catch-up position, on `conn`. A no-op when absent.
