...
/**
 * Shell-level URL-driven mount for editing a management-fee deduction (FEE-063).
 *
 * Subscribes to URL search params
 * (`modal=edit-management-fee&editTxId=…&editTxAccountId=…&editTxAssetId=…`) and
 * overlays the dedicated management-fee edit modal (date/quantity/note editable,
 * asset locked). The transaction list (a sibling feature) opens it by mutating URL
 * params only — no cross-feature import at the call site (B13). The transaction is
 * (re)fetched here via the per-asset list command, then handed to the modal as the
 * prefill source.
 */
export function ManagementFeeEditModalMount() {
  ...
  const active = modal === "edit-management-fee" && !!editTxId && !!accountId && !!assetId;
