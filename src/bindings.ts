 ...
 */
async getAccountDetails(accountId: string, asOfDate: string | null) : Promise<Result<AccountDetailsResponse, AccountError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("get_account_details", { accountId, asOfDate }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
