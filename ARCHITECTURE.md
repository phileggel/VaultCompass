...
use_cases/shared/     stateless helpers shared by use cases without importing one another (B18)
  global_value.rs     the single Global Value arithmetic + REFERENCE_CURRENCY (PMV-023, GPF-011)
  price_movement.rs   pure Price Movement report builder (PMV-020/022/030-060)
...
| `AssetPriceUpdated`        | `context/asset/` price writes (including a price history backfill, MKT-194) + `use_cases/asset_price_fetch/`                       | `account_details`, `performance` (per-page)                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `HoldingNoteUpdated`       | `context/account/` holding-note upsert / delete, `apply_holding_note`, and an applied note removal (SYN-064)                       | `account_details`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `CurrencyPairUpdated`      | `context/currency/` pair declare, `apply_currency_pair`, and pair removal (SYN-064)                                                | `currency_rates_view`, `account_details`                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `AssetPriceFetchCompleted` | `use_cases/asset_price_fetch/` dispatcher (bulk fetch end, MKT-181)                                                                | `account_details`, `accounts`, `performance` (per-page); progress bar cleared + unpriced list refreshed via `lib/store.ts`. Carries a `PriceMovementReport` when a user-started Global refresh produced one (PMV-010); the accounts-list dialog reads it through its own mount-scoped listener, NOT `lib/store.ts`, so the report is discarded on navigation (PMV-016). `AccountManager` reads `lib/store.ts` only for the unpriced slice, to hold the report back while the manual-fill modal is up (PMV-018) |
| `SyncCompleted`            | `context/sync/` `SyncService::remember_run` — a run that applied changes or changed the device's failures / paused state (SYN-064) | bare marker: `lib/store.ts` re-fetches assets, categories, accounts; `shell/sync_indicator` re-reads `get_sync_status`                                                                                                                                                                                                                                                                                                                                                                                         |
