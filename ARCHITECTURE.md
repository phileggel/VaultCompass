...
| `AssetPriceUpdated`        | `context/asset/` price writes + `use_cases/asset_price_fetch/`              | `account_details`, `account_performance` + `global_performance` (per-page)                                                                                |
| `AccountUpdated`           | `context/account/` account writes                                           | `accounts`, `account_performance` + `global_performance` (per-page)                                                                                       |
| `TransactionUpdated`       | `context/account/` holding / transaction writes                             | `account_details`, `transactions`, `account_performance` + `global_performance` (per-page)                                                                |
...
| `AssetPriceFetchCompleted` | `use_cases/asset_price_fetch/` dispatcher (bulk fetch end, MKT-181)         | `account_details`, `accounts`, `account_performance` + `global_performance` (per-page); progress bar cleared + unpriced list refreshed via `lib/store.ts` |
