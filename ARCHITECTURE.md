...
| `AssetPriceFetchCompleted` | `use_cases/asset_price_fetch/` dispatcher (bulk fetch end, MKT-181)                                                                | `account_details`, `accounts`, `performance` (per-page); progress bar cleared + unpriced list refreshed via `lib/store.ts`. Carries a `PriceMovementReport` when a user-started Global refresh produced one (PMV-010); the accounts-list panel reads it through its own mount-scoped listener, NOT `lib/store.ts`, so the report is discarded on navigation (PMV-016) |
